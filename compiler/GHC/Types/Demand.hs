...
_botCard, _absCard, topCard :: Card
_botCard = C_10
_absCard = C_00
topCard = C_0N
...
polyDmd :: Card -> Demand
polyDmd C_00 = D C_00 Unboxed poly00
...
polyDmd C_10 = D C_10 Unboxed poly10
...
lubDmd d1@(D n1 b1 sd1) d2@(D n2 b2 sd2)
  | C_10 <- n1 = d2
  | C_10 <- n2 = d1
  | otherwise  = D (lubCard n1 n2) (lubBoxity b1 b2) (lubSubDmd sd1 sd2)
...
plusDmd d1@(D n1 b1 sd1) d2@(D n2 b2 sd2)
  | C_00 <- n1 = d2
  | C_10 <- n1 = botDmd
  | isAbs n2   = plusDmd d2 d1 -- Reflexivity
  | otherwise  = D (plusCard n1 n2) (plusBoxity b1 b2) (plusSubDmd sd1 sd2)
...
dmdTransformDictSelSig (DmdSig (DmdType _ [D _ _ prod] _)) call_sd
   | (n, sd') <- peelCallDmd call_sd
   , Prod sig_ds <- prod
   = multDmdType n $
     DmdType emptyDmdEnv [D C_11 Unboxed (Prod (map (enhance sd') sig_ds))] topDiv
   | otherwise
   = nopDmdType -- See Note [Demand transformer for a dictionary selector]
  where
    enhance sd old@(D n _ _) | isAbs n   = old
                             | otherwise = D n Unboxed sd  -- This is the one!
